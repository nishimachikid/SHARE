...
debug('カテゴリデータ：'.print_r($dbFormData,true));
...
if(!empty($_POST)){
  ...
  // 更新の場合はDBno情報と入力情報が異なる場合にバリデーションを行う。
  if(empty($dbFormData)){
    //未入力チェック
    ...
    if($dbFormData['prcie'] != $price ){ //前回まではキャストしていが、ゆるい判定でもいい
    //未入力チェック
      validRequired($price, 'price');
      //半角数字チェック
      validNumber($price, 'price');
  ...
  if(empty($err_msg)){
    ...
    try {
      ...
      $dbh = dbConnect();
      //SQl文作成
      // 編集画面の場合はUPDATE文、新規登録画面の場合はINSERTぶんを生成
      if($edit_flg){
        debug('DB更新です。');
...
?>
<?php
$siteTitle = (!$edit_flg) ? 'コンテンツ登録' : 'コンテンツ編集';
require('head.php');
?>
  ...
  <body class="page-profEdit page-2colum page-logined">
    ...
    <div id="contents" class="site-width">
      <h1 class="page-title"><?php echo (!$edit_flg) ? '登録する' : '編集する'; ?></h1>
      <!-- Main -->
      <section id="main" >
