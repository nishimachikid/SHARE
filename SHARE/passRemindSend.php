    ...
    if(empty($err_msg)){
      ...
      try {
        ...
        if($stmt && array_shift($result)){
          ...
          $auth_key = makeRandKey(); //認証キー
          
          //メールを送信
          $from = 'info@webukatu.com';
          $to = $email;
          $subject = '【パスワード再発行認証】｜　SHARE';
...
パスワード再発行認証キー入力ページ：http://localhost:8888/portfolio/passRemindRecieve.php
認証キー：{$auth_key}
※認証キーの有効期限は30分となります

認証キーを再発行されたい場合は下記ページより再度発行をお願いします。
http://localhost:8888/portfolio/passRemindSend.php
